...
#mydata = read.csv("course_proj.csv", header = T, sep = ";")
...
server = function(input, output, session){
  ...
  output$tableDT <- DT::renderDataTable(DT::datatable(course_proj) %>%
                                          formatCurrency("MarketCap_in_M", "$", digits = 0) %>%
  ...
  output$scat = renderPlot({
    ggplot(weighted.mydata(), aes(points, MarketCap_in_M)) +
...
ui = navbarPage(theme = shinytheme("sandstone"), title = h3("The Mining Stock Scale"),
                ...
                tabPanel("Documentation",
                         ...
                         tags$iframe(style="height:315px; width:560px",
